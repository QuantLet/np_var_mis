...
generated quantities {
  vector[N] latent_f;
  vector[N] latent_noise;
  real predicted[N];
  for (j in 1:N){
    latent_f[j] = la_1+(1-la_1)/(1+10^(slope_1*(x_test[j]-log10_ec50_1)));
  }
  latent_noise = s*sqrt((latent_f+lambda));
  predicted = normal_rng(latent_f,latent_noise);
}
